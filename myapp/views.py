    ...
    if request.method == "POST":
        ...
        if date_form.is_valid():
            ...
            if start_date > end_date:
                ...
                result = Motor.objects.filter(
                        delivery_motor__invoice__time__range=(start_date, end_date)
                    ).annotate(
                        month_sale=TruncMonth('delivery_motor__invoice__time'),
                        total_quantity=Sum('delivery_motor__quantity')
                    ).values(
                        'month_sale', 'image', 'name', 'total_quantity'
                    ).order_by('month_sale', '-total_quantity')
...
def getMotorByAPI(request):
    search = request.GET.get('search')
    motor = Motor.objects.all()
    if search: 
        motor = motor.filter(Q(name__icontains=search) | Q(description__icontains=search))
    paginator_motor = Paginator(motor, 5)
    page_number = request.GET.get('page')
    motor_list = paginator_motor.get_page(page_number)
    current_page_number = page_number
    serialized_motor = MotorSerializer(motor_list, many=True)
    data = {
        'count': motor.count(),
        'motors' : serialized_motor.data,
